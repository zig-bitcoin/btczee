...
pub fn receiveMessage(
    ...
    else if (std.mem.eql(u8, &command, protocol.messages.AlertMessage.name()))
        protocol.messages.Message{ .alert = try protocol.messages.AlertMessage.deserializeReader(allocator, r) }
    else {
