...
pub fn receiveMessage(
    ...
    else if (std.mem.eql(u8, &command, protocol.messages.SubmitOrderMessage.name()))
        protocol.messages.Message{ .submitorder = try protocol.messages.SubmitOrderMessage.deserializeReader(allocator, r) }
    else {
