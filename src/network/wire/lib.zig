...
pub fn receiveMessage(
    ...
    else if (std.mem.eql(u8, &command, protocol.messages.CheckOrderMessage.name()))
        protocol.messages.Message{ .checkorder = try protocol.messages.CheckOrderMessage.deserializeReader(allocator, r) }
    else {
