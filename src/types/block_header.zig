...
pub fn serializeToWriter(self: *const Self, w: anytype) !void {
    ...
    try w.writeInt(u32, self.timestamp, .little);
    try w.writeInt(u32, self.nbits, .little);
    try w.writeInt(u32, self.nonce, .little);
}
...
pub fn deserializeReader(allocator: std.mem.Allocator, r: anytype) !Self {
    ...
    header.timestamp = try r.readInt(u32, .little);

    header.nbits = try r.readInt(u32, .little);
    header.nonce = try r.readInt(u32, .little);
